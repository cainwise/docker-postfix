...
#### PTR 记录
PTR记录，是电子邮件系统中的邮件交换记录的一种，也就是 IP 反向解析，通过设置 PTR 可以提高发信方信誉，提高送达率。
...
| PTR    | 1.2.3.4           | mail.m31271n.com |

## 避免你的邮件被当成垃圾邮件

SPF 和 DKIM 是两种邮件验证机制，之所以需要额外的验证机制，是因为 Email 的发送地址很容易伪造。
...
| Type   | Host              | Answer                         |
|--------|-------------------|--------------------------------|
| TXT    | m31271n.com       | `v=spf1 ip4:<发信 IP> ~all`    |
| SPF    | m31271n.com       | `v=spf1 ip4:<发信 IP> ~all`    |
